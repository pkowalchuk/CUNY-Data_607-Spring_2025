...
# untidy_flights <- read_csv("https://raw.githubusercontent.com/scrummett/DATA607/refs/heads/main/UntidyFlights.csv")
```
...
untidy_flights <- untidy_flights |> 
  rename("Airline" = "X",
         "Status" = "X.1") 
  # mutate(acroos(Los_Angeles:Seattle, ~as.numeric(gsub(",", "", .x)))) |>
  # filter(!if_all(Los_Angeles:Seattle, is.na)) |>   # More concise NA filtering
  # filter(Airline, .direction = "down")   # Explicit fill direction
untidy_flights <- untidy_flights |> 
...
## Additional Analysis: Delay Status by City and Airline

*Added by Alina Vikhnevich*

Now that the dataset is cleaned and we’ve compared delay percentages overall, it’s useful to break down **delay status across cities for each airline**.

This visualization highlights which cities contribute most to on-time or delayed performance.

```{r alina_city_delay_plot, warning=FALSE, message=FALSE}
# Plot delay status by city and airline
tidy_flights %>%
  ggplot(aes(x = City, y = n_Flights, fill = Status)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~ Airline) +
  scale_fill_manual(values = c("on time" = "steelblue", "delayed" = "tomato")) +
  labs(
    title = "Flight Delay Status by City and Airline",
    x = "City", y = "Number of Flights", fill = "Status"
  ) +
  coord_flip() +
  theme_minimal()
```
